...
```{r scrapePackages, collapse=TRUE}
library(XML)
library(RCurl)
library(stringr)
```

Each of these packages will help us do different things. The XML package has really helpful functions for turning websites into useable R data. However, the XML package doesn't play nice with https, so we'll have to use another package to tell R that it doesn't have to worry about security--that is, the RCurl package. 

The last package that we just loaded is called "stringr", and it's super useful for cleaning up data. Sometimes, there will be extra whitespaces or numbers or even dogs, and we want to clean the data up so it's not weighed down by noise.

We have an object telling R where to go on the internet, so let's store the URL into an object: 
```{r makeURL, collapse=TRUE}
url <- "https://en.wikipedia.org/wiki/Popular_cat_names"
```

Wait a minute! This website looks like it uses https! That's a bit annoying, but we can work around it. If we want to scrape data from an https website, we'll have to tell R not to use the extra layer of security.

Fortunately, the RCurl package has a function that lets you choose not to use https. Phew! Here, we'll tell R to copy the webpage into a new object, "wikiCatNamePage". 
```{r makePage, collapse=TRUE}
wikiCatNamePage <- getURL(url,.opts = list(ssl.verifypeer = FALSE) )
```

Cool! We have the website in an object!

But...looking at the object, it's just a bunch of gibberish! This is where we use the XML package--it's really good at translating what the HTML is trying to say! That's what we'll do in this next command:

```{r htmlParse, collapse=TRUE}
parsedWikiCatNamePage <- htmlParse(wikiCatNamePage)
```

Alright! Now we've organized the website so that a different function, htmlTables(), will be able to pull the tables that we want. 

```{r tableParse, collapse=TRUE}
wikiCatNameTables <- readHTMLTable(parsedWikiCatNamePage)
```

It looks like the function stored seven different things in the object, "wikiCatNameTables". Looking at the webpage, I like the table from the subsection called "Cultural references to the naming of cats". One of my uncles is related to the son-in-law of the cousin of the co-creator of Tom, from Tom and Jerry. I know--I'm cool. 

If we use the str() function to look at the objects in wikiCatNameTables, we'll see that the last item in the list has the data we want. The str() function shows too much information because of how wikiCatNameTables is organized, so the output isn't shown here. You should try it, though!

So, let's put the last item in wikiCatNameTables into an object and see what we get!

```{r makeFamous, collapse=TRUE}
